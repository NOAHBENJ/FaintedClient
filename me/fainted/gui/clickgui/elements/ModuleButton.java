...
import me.fainted.gui.clickgui.Panel;
import me.fainted.gui.clickgui.elements.menu.ElementCheckBox;
...
public class ModuleButton {
	 ...
	 * Konstrukor
	 */
	public ModuleButton(Module imod, Panel panel) {
		mod = imod;
		 ...
		 */
		if (Fainted.instance.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Fainted.instance.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Fainted.clickGUI.closeAllSettings();
				this.extended = b;
	...
	public boolean keyTyped(char typedChar, int keyCode) throws IOException {
		...
		if (listening) {
			if (keyCode != Keyboard.KEY_ESCAPE) {
				//Fainted.sendChatMessage("Bound '" + mod.getName() + "'" + " to '" + Keyboard.getKeyName(keyCode) + "'");
				mod.setKey(keyCode);
			} else {
				//Fainted.sendChatMessage("Unbound '" + mod.getName() + "'");
